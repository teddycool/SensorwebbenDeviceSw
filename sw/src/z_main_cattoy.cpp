 ...
 * This project contains the software for sensor-device 'Cat Toy Mini' built around ESP8266.
 ...
 *
 * SW v3.0.0 2025-01-20
 * License: GPL-3.0
 ******************************************************************************
 * History:
 * ------------ *
 * v 3.0.0:
 * Initial version based on Misto template but using ADXL345 accelerometer
 * instead of DHT temperature/humidity sensor for motion detection.
 * Includes motor control for cat toy functionality.
 */
#define CHIPTYPE ESP8266 // Define the chip type as ESP8266
#include "CatToyEsp8266Config.h"
...
// ===== Motor helper =====
static void motorKick(uint16_t duty /*0..1023*/, uint16_t ms) {
...
void setup()
{
  ...
  if ((MQTT_LOCAL == true) && (digitalRead(MODEPIN) == HIGH))
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      // Send discovery messages for ADXL345 acceleration data
      dclient.sendAccelX();
      dclient.sendAccelY();
      dclient.sendAccelZ();
      dclient.sendAccelMagnitude();
      dclient.sendAccelXYForce();
      ...
      else
      {
        ...
        if (size > 0)
        {
          ...
          if (!deserializeError)
          {
            ...
            // Read ADXL345 acceleration data instead of DHT
            Serial.println("Reading ADXL345 acceleration sensor...");
            Adxl345Sensor adxlSensor(I2C_SDA, I2C_SCL);
            if (adxlSensor.performMeasurement()) {
              Serial.println("ADXL345 sensor measurement successful");
              adxlSensor.addToPayload(mqttpayload);
              
              // Get acceleration vector for potential motor control logic
              AdxlVector accel = adxlSensor.getAcceleration();
              Serial.printf("Acceleration: X=%.1f, Y=%.1f, Z=%.1f mg\n", 
                           accel.x, accel.y, accel.z);
              
              // Enhanced motion detection using both total and XY force
              float total_magnitude = sqrt(accel.x * accel.x + accel.y * accel.y + accel.z * accel.z);
              float xy_force = sqrt(accel.x * accel.x + accel.y * accel.y);
              
              Serial.printf("Forces: Total=%.1f mg, XY=%.1f mg\n", total_magnitude, xy_force);
              
              // Motion detection thresholds
              const float TOTAL_MOTION_THRESHOLD = 1200; // Overall motion threshold
              const float XY_TAP_THRESHOLD = 800;        // Horizontal tap threshold
              
              bool motion_detected = false;
              if (total_magnitude > TOTAL_MOTION_THRESHOLD) {
                Serial.println("Strong motion detected! Activating motor...");
                motorKick(800, 250); // Longer activation for strong motion
                motion_detected = true;
              } else if (xy_force > XY_TAP_THRESHOLD) {
                Serial.println("Horizontal tap detected! Quick motor response...");
                motorKick(600, 150); // Shorter, gentler activation for taps
                motion_detected = true;
              }
              
              mqttpayload["motion_detected"] = motion_detected;
              mqttpayload["total_force"] = total_magnitude;
              mqttpayload["xy_force"] = xy_force;
            } else {
              Serial.println("ADXL345 sensor measurement failed");
              mqttpayload["accel_error"] = "sensor_failed";
              mqttpayload["motion_detected"] = false;
            }
            
            Serial.println("ADXL345 sensor data added to payload");
