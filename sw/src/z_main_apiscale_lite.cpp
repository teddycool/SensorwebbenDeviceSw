 ...
 * This project contains the setup-software for sensor-device 'APIScale Lite', built around ESP8266.
...
** 6: test HX711 weight sensor functionality
...
// HX711 pin definitions for ESP8266
#define LOADCELL_DOUT_PIN 14 // HX711 DOUT pin
...
float weightCalFactor; // Calibration factor for weight sensor
String mqtt_ptopic;
String mqtt_dtopic;
...
void setup()
{
    ...
    Serial.println();
    Serial.println();
    Serial.println("=== APISCALE LITE SETUP STARTING ===");
...
void loop()
{
    ...
    Serial.println("Powering up HX711...");
    delay(2000); // Allow HX711 to power up properly
    
    if (hx711Sensor.performMeasurement())
    {
        Serial.println("HX711 sensor measurement successful");
        hx711Sensor.addToPayload(mqttpayload);
        
        // Calculate a basic weight calibration factor (you'll need to adjust this based on known weights)
        // This is a placeholder - in production you'd use a known weight for calibration
        weightCalFactor = 1.0; // Default scale factor
        Serial.println("Weight calibration factor: " + String(weightCalFactor));
    }
    else
    {
        Serial.println("HX711 sensor measurement failed - check wiring!");
        mqttpayload["hx711_error"] = "Sensor not ready or connection failed";
    }
